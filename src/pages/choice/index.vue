  ...
  <div class="main">
    ...
    <scroll-view>
      <div class="content" v-if="!time">
        <swiper previous-margin="15px" next-margin="15px" interval="10000" autoplay @change="change($event)">
          <block v-for="(val,key) in data" :key="key">
            <swiper-item class="item" :class="{'current-item':current==key}">
              <image lazy-load="true" :style="{background:val.color}" :src="val.urls.small" class="slide-image" mode="aspectFill"
                     :data-value="val.urls.small" @click="browser($event,key)">
                     </image>
        ...
        <!-- 列表 -->
        <div class="list">
          <div class="list-item flex" v-for="(val,key) in list" :key="key">
              <image :src="val.urls.small" :style="{background:val.color,height:val.width/val.height*169+'px'}" mode="aspectFill" 
                   :data-value="val.urls.small"
                   @click="browser($event,key)"></image>
              <!-- <span class="list-title" v-text="val.description"></span> -->
              <div class="setting">
                <div class="setting-first" @click="like(key)">
                  <div class="like icon">
...
<style scoped>
  ...
  .header-bg{
    position: absolute;
    width: 100%;height: 7.5rem;display: flex;left: 0px;top: -40px;opacity: 0.6;box-shadow: 1px 2px 50px rgba(255,255,255,0.5)
  }
  .header-bg image{
    width: 100%;height: 7.5rem;display: flex;filter:blur(50px);
  }
  ...
  .current-item i{position: absolute;width: 100%;height: 5rem;left: 0px;top: 0px;}
  ...
  /* list */
  .list{justify-content: space-between;padding: 0.25rem;margin-top: 10px;box-sizing: border-box;position: inherit;z-index: 100;display:flex;background: rgba(0, 0, 0, 0);flex-wrap: wrap}
  .list-item{width: 3.38rem;position: relative;margin-bottom: 10px;align-content:flex-start;}
  .list-item i{width: 100%;height: auto;position: absolute;left: 0px;top: 0px;}
  .list-item image{width: 100%;border-radius: 2px;position:initial;
z-index:10;}

  .setting{width: 100%;font-size: 14px;color: #999;box-sizing: border-box;display:flex;justify-content:space-between;background: #fff;margin-top: 5px}
  .list-title{font-size: 14px;color: #999;}
  .icon image{width: 0.3rem;height: 0.3rem;margin: auto}

            .setting-first{display: flex;justify-content: flex-start;}
            .like{
              display: flex;
            }
            .share{
            ...
            .download  image{width: 0.35rem;height: 0.35rem;}
  
  .loading{
        height: 50px;
        width: 100%;display: flex;
      }
  .loading  image{margin: auto;height: 50px;}
